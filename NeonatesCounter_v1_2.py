...
def main():
    # Define the folder that is two levels up from this script
    script_path = Path(__file__).parent
    model_path = script_path / "NeonatesCounter_v1.0_model.pt"
    two_levels_up_folder = script_path.parent.parent
    print(two_levels_up_folder)
    
    # Get the directories created today
    today = pd.Timestamp.today().strftime('%Y-%m-%d')
    today_folders = []

    for folder in two_levels_up_folder.iterdir():
        if folder.is_dir():
